...
function EmailApp.new()
    ...
    -- Master Email Database - START EMPTY, emails are injected dynamically
    self.allEmails = {}
    ...
    self:loadInjectedEmails()
    
    return self
end


-- Load previously injected emails (silently, without notifications)
function EmailApp:loadInjectedEmails()
    local ContentRegistry = require("src.core.content_registry")
    local injected = ContentRegistry.getInjectedEmailData()
    for _, emailData in ipairs(injected) do
        -- Use silent add to avoid duplicate notifications
        self:addEmail(emailData, true)
    end
end

-- Add email with optional silent parameter
function EmailApp:addEmail(emailData, silent)
    silent = silent or false
    
    -- Ensure email has all required fields
    emailData.id = emailData.id or (#self.allEmails + 1)
    emailData.unread = emailData.unread ~= nil and emailData.unread or true
    
    if emailData.unlocked == nil then
        emailData.unlocked = true
    end
    
    -- Check if email already exists
    for i, existing in ipairs(self.allEmails) do
        if existing.id == emailData.id then
            if not existing.unlocked and emailData.unlocked then
                self.allEmails[i] = emailData
                if not silent then
                    AudioManager.playSFX("notification", 1.2)
                    Notifications.add(
                        "New Email",
                        "From: " .. emailData.sender .. " - " .. emailData.subject,
                        nil,
                        5.0
                    )
                end
    ...
    -- Add new email
    table.insert(self.allEmails, emailData)
    ...
    -- Resort emails (newest first by id)
    table.sort(self.allEmails, function(a, b) 
...
-- Keep the event listener as is (it already uses addEmail without silent)
EventBus.on("email:incoming", function(emailData)
    ...
    for _, win in ipairs(WindowManager.openApps) do
        if win.app and win.app.name == "Email" and win.instance then
            win.instance:addEmail(emailData, false)  -- false = with notification
        end
    end
end, "email_dynamic_inject")

function EmailApp:getVisibleEmails()
    local visible = {}
    for _, email in ipairs(self.allEmails) do
        -- Only show unlocked emails
        if email.unlocked then
...
-- (draw, mouse handlers, etc. - no changes needed)
...
function EmailApp:handleDetailInteractions(detailWidth, padding, emails)
    ...
    if isCompact then        starY = starY + 30
    end
