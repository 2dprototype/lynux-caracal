...
function Taskbar.drawTopBar()
    ...
    -- Top bar minimalist dark slate background
    love.graphics.setColor(0.09, 0.11, 0.15, 0.96)
    ...
    love.graphics.circle("fill", 16, h / 2, 4)
    
    love.graphics.setFont(Taskbar.boldFont)
    love.graphics.setColor(0.9, 0.94, 0.98)
    love.graphics.print("Lynux", 26, 6)

    -- Player Level & XP Pill (Compact, Professional)
    ...
    if task then
        ...
        local taskBadgeW = 210
        if task.completed then
            love.graphics.setColor(0.1, 0.22, 0.16, 0.9)
        else
            love.graphics.setColor(0.2, 0.16, 0.1, 0.9)
        end
        love.graphics.rectangle("fill", taskBadgeX, badgeY, taskBadgeW, badgeH, 4, 4)
        
        if task.completed then
            love.graphics.setColor(0.2, 0.85, 0.4, 0.8)
        else
            love.graphics.setColor(1.0, 0.72, 0.22, 0.8)
        end
    ...
    if Taskbar.storyBtnHover then
        love.graphics.setColor(0.2, 0.5, 0.8, 0.95)
    else
        love.graphics.setColor(0.14, 0.19, 0.28, 0.9)
    end
    love.graphics.rectangle("fill", btnX, btnY, btnW, btnH, 4, 4)
    love.graphics.setColor(0.3, 0.75, 1.0, 0.7)
...
function Taskbar.mousepressed(x, y, button)
    ...
    -- Top bar Story Mode button click
