...
local Notifications = {
    list = {}, -- array of { id, title, message, icon, timer, duration, onClick }
    maxCount = 4
}
...
function Notifications.draw()
    ...
    for _, n in ipairs(Notifications.list) do
        ...
        -- Panel background
        love.graphics.setColor(0.1, 0.13, 0.18, 0.96 * n.alpha)
        love.graphics.rectangle("fill", notifX, notifY, n.width, n.height, 5, 5)

        -- Retro Azure Border
        love.graphics.setColor(0.2, 0.75, 1.0, 0.85 * n.alpha)
        love.graphics.setLineWidth(1.2)
        love.graphics.rectangle("line", notifX, notifY, n.width, n.height, 5, 5)

        -- Accent Dot
        love.graphics.setColor(0.2, 0.88, 0.55, n.alpha)
        love.graphics.circle("fill", notifX + 16, notifY + 16, 4)

        -- Title
        love.graphics.setColor(0.96, 0.98, 1.0, n.alpha)
        love.graphics.print(n.title, notifX + 26, notifY + 8)

        -- Message
        love.graphics.setColor(0.75, 0.8, 0.88, 0.9 * n.alpha)
